local function register_functions()
	...
	vim.fn["remote#host#RegisterPlugin"]("node", path .. "/node/command-server/", {
		{
			type = "function",
	...
	vim.fn["remote#host#RegisterPlugin"]("node", path .. "/node/cursorless-neovim/", {
		{
			type = "function",
...
local function configure_command_server_shortcut()
	...
	vim.api.nvim_set_keymap("i", "<C-S-F12>", "<cmd>lua vim.fn.CommandServerRunCommand()<CR>", { noremap = true })
	vim.api.nvim_set_keymap("n", "<C-S-F12>", "<cmd>lua vim.fn.CommandServerRunCommand()<CR>", { noremap = true })
	vim.api.nvim_set_keymap("c", "<C-S-F12>", "<cmd>lua vim.fn.CommandServerRunCommand()<CR>", { noremap = true })
	vim.api.nvim_set_keymap("v", "<C-S-F12>", "<cmd>lua vim.fn.CommandServerRunCommand()<CR>", { noremap = true })
	vim.api.nvim_set_keymap("t", "<C-S-F12>", "<cmd>lua vim.fn.CommandServerRunCommand()<CR>", { noremap = true })
end

local function setup()
	if not vim.g.cursorless_warned then
		if vim.fn.has("nvim-0.10") == 0 then
			vim.notify("cursorless.nvim only tested on >= nvim-0.10", vim.log.levels.WARN)
			return
		end
		vim.g.cursorless_warned = true
	end
	register_functions()
	load_extensions()
